...
移动端适配采用 [postcss-px-to-viewport]，使用的时候按照设计稿，在postcss.config中找到"viewportWidth"设置宽度（个人设置为375，这样引入组件库的时候都会按照这个转换，就不用了引入组件之后都要更改样式了），插件会自动实现vw的转换。
...
```
1、查看设计稿的宽度，如750px、375px。
2、样式、css等就可以按照设计稿的大小用px来写了
```
